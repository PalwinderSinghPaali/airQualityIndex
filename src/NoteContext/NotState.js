...
function NotState(props) {
  ...
  const setDataOn = async (deviceID,deviceName) => {
      const res1 = await axios.get(`YOUR API URL /deviceData?deviceId=${deviceID}&start=0&limit=${postsPerPage}
      `);
      setDeviceId(deviceID);
      const url1=`YOUR API URL /deviceData?deviceId=${deviceID}`;
      setUrl(url1);
    ...
    const searchData = async (search) =>{
      const res1 = await axios.get(`YOUR API URL /deviceList/${search}`);
      console.log(res1.data)
      if(res1.data.status==='success'){
        console.log('success');
        const ndata = res1.data.data;
    ...
    const sortData = async (deviceID,orderBy,sortOrder,x)=>{
      const url1=`YOUR API URL /deviceData?deviceId=${deviceID}&orderBy=${orderBy}&sortOrder=${sortOrder}`
  ...
  const sortFilterData = async (deviceID,orderBy,sortOrder,StartDate,EndDate,x)=>{
    const res1 = await axios.get(`YOUR API URL /deviceData?deviceId=${deviceID}&orderBy=${orderBy}&sortOrder=${sortOrder}&startDate=${StartDate}&endDate=${EndDate}&start=${x*postsPerPage}&limit=${postsPerPage}
     `);
    console.log(res1.data)
    setFilterUrl(`YOUR API URL /deviceData?deviceId=${deviceID}&orderBy=${orderBy}&sortOrder=${sortOrder}&startDate=${StartDate}&endDate=${EndDate}`)
    if(res1.data.status==='success'){
      console.log('success');
      const ndata = res1.data.data;
      const total = res1.data.count;
  ...
  //   // const res1 = await axios.get(`YOUR API URL /deviceData?deviceId=${deviceID}`);
  //   // console.log(res1.data)
  //   // if(res1.data.status==='success'){
  //   //   console.log('success');
  //   //   const ndata = res1.data.data;
...
const whetherDateFiltered = async (StartDate,EndDate,deviceID,x)=>{
  ...
  const res1 = await axios.get(`YOUR API URL /deviceData?deviceId=${deviceID}&startDate=${StartDate}&endDate=${EndDate}&start=${x*postsPerPage}&limit=${postsPerPage}
  `);
  console.log(res1.data)
  const url1=`YOUR API URL /deviceData?deviceId=${deviceID}&startDate=${StartDate}&endDate=${EndDate}`;
  setFilterUrl(url1);
  if(res1.data.status==='success'){
    console.log('success');
    const ndata = res1.data.data;
